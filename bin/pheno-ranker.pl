...
sub pheno_ranker {
    ...
    my $share_dir   = dist_dir('Pheno-Ranker');
...
# NB: Darwin does not have nproc to show #logical-cores, using sysctl instead
    ...
    my $info = {
        ...
        version  => $VERSION,
        user     => $ENV{'LOGNAME'} || $ENV{'USER'} || $ENV{'USERNAME'} || 'dummy-user'
    };
...
Download a docker image (latest version) from L<Docker Hub|https://hub.docker.com/r/manuelrueda/pheno-ranker> by executing:
